...
function leftDeepCompare(a, b) {
    ...
    for (var i = 0; i < aprops.length; i++) {
        if (!b.hasOwnProperty(aprops[i].key)) {
            return false;
        }
        if (Array.isArray(aprops[i].value)) {
            if (!aprops[i].value.includes(b[aprops[i].key])) {
                return false;
            }
        } else if (!leftDeepCompare(aprops[i].value, b[aprops[i].key])) {
            return false;
        }
    }
